...
[Magnetic Resonance Spectroscopy (MRS)](https://bids-specification.readthedocs.io/en/stable/modality-specific-files/magnetic-resonance-spectroscopy.html).
This tutorial converts MRS data stored as DICOM into a BIDS dataset, then views
...
> [!TIP]
> BIDSvue's built-in dcm2niix can only convert MRS data that's stored as DICOM.
> If your scanner saves data in a
> [proprietary format](https://github.com/wtclarke/spec2nii#currently-supported-formats),
> use [spec2nii](https://github.com/wtclarke/spec2nii) instead.
...
Now inspect one of the MRS files. For example, `run-01_svs` offers an `image`
view that plots signal intensity against the chemical shift — measured in parts
per million (ppm) along the x-axis. Each metabolite appears as a distinct peak or
multiplet at a specific ppm, set by its chemical environment.
