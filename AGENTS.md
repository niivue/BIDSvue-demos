...
- **Generated-asset promotion guard.** Image editors and visual iteration may replace files under `dist/assets/` before the source copy is updated (this previously lost an improved radar mesh). After the first build in a checkout, the ignored root `.asset-manifest.json` records generated hashes; a later build refuses to overwrite changed generated assets until they are copied into `assets/`. With no manifest, `dist/` is treated as stale output and rebuilt normally.
