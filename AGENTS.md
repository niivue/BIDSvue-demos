...
- **`scripts/dev.ts`** — dev server on the first available port from `localhost:5173`, SSE live-reload (`/__livereload`), and a source file watcher that rebuilds + reloads. An explicit `PORT` remains strict.
...
- **`dev.ts` content-signature gate and serialized rebuilds.** macOS `fs.watch` fires on access-time changes, and the build reads the very files it watches — so a naive rebuild-on-event loops forever. The watcher only rebuilds when a signature of `path:mtime:size` over the watched sources actually changes. Builds are serialized because each one replaces the shared `dist/` tree; edits arriving during a build coalesce into one follow-up build.
