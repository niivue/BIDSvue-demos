...
- **`scripts/build.ts`** — Markdown + `site.config.ts` → `dist/`. Emits `dist/index.html` (hero + tutorial cards + tools), `dist/about/index.html` (project/legitimacy page + JSON-LD provenance), `dist/<slug>/index.html` per tutorial, a self-contained `dist/404.html` (inlined CSS so it renders at any Pages depth), plus `robots.txt` + `sitemap.xml` (crawlability), copies each tutorial's screenshots and `assets/`, writes `.nojekyll`. Before replacing output, preflight checks fail on missing local CSS/tutorial assets, and a SHA-256 manifest blocks canonical `dist/assets` edits that have not been promoted back into `assets/`. Reads each figure's PNG size (`pngSize`) so `<img>` carries intrinsic `width`/`height` → no layout shift.
...
- **`scripts/dev.ts`** — dev server on the first available port from `localhost:5173`, SSE live-reload (`/__livereload`), and a source file watcher that rebuilds + reloads. Each process serves an isolated temporary build tree so fallback-port instances cannot race over `dist/`. An explicit `PORT` remains strict.
...
- **`scripts/build.test.ts`** — generated-site contract, metadata, asset guards, isolated-build safety/recovery, and hero-pair registry tests.
...
- `bun run dev` — build + serve from `localhost:5173` with live reload, incrementing to the next free port when the default is busy; auto-opens a browser (`NO_OPEN=1` to skip). `PORT` overrides the port and remains strict.
...
- `bun run test` — `bun test` (runs `scripts/*.test.ts`). It invokes a canonical `build()`, so it rewrites ignored `dist/` and `.asset-manifest.json`; promote intentional `dist/assets` experiments into `assets/` before testing.
...
Home visual and persistent site chrome:

- **Home hero radar**: a pre-paint script randomly chooses the mesh, axial voxel, coronal voxel, sagittal voxel, or multi-view surface-render base/activation pair on every load. Variant IDs and captions live in the `HERO_VARIANTS` registry in `build.ts`; `site.css` maps each ID to its source images and framing. Wide image montages sweep 0–100%, while the cortical mesh keeps its tighter 18–82% transit. A pointer click is deferred briefly so double-click (or Shift+Enter) can cycle variants without disturbing the animation phase; the radar pauses while offscreen and resumes from the same phase. Runtime reduced-motion changes add or remove its interactive semantics. Activation-mask PNGs carry luminance in their alpha channel for cross-browser masking and a build test enforces matching pair dimensions and alpha-bearing active masks.
...
- **`dev.ts` content-signature gate, isolated output, and serialized rebuilds.** macOS `fs.watch` fires on access-time changes, and the build reads the very files it watches — so a naive rebuild-on-event loops forever. The watcher only rebuilds when a signature of `path:mtime:size` over the watched sources actually changes. Each dev process builds into its own temporary tree, and edits arriving during one process's build coalesce into one follow-up build. Builds must not write into the watched `assets/` or tutorial roots; if that changes, exclude the generated paths or the watcher will rebuild itself indefinitely.
...
- **Localhost-only service-worker cleanup.** `theme.js` unregisters service workers only on `localhost` / `127.0.0.1` because a stale worker from another app can hijack a reused dev port and freeze this site. Never broaden it to production origins, where unregistering a shared-origin worker could break another project; do not remove it as apparently unused code.
- **Generated-asset promotion guard.** Canonical `bun run build` / `bun run preview` workflows may replace files under `dist/assets/` during visual iteration before the source copy is updated (this previously lost an improved radar mesh). After the first canonical build in a checkout, the ignored root `.asset-manifest.json` records generated hashes; a later canonical build refuses to overwrite changed generated assets until they are copied into `assets/`. `bun run dev` instead serves a disposable temporary tree, so visual edits must be made in `assets/`. With no manifest, `dist/` is treated as stale output and rebuilt normally.
